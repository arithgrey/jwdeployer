...
class DockerComposeStopper:
    def __init__(self, base_directory, docker_compose_path="docker-compose"):
    ...
    def run_docker_compose_down_with_volumes(self):
        ...
        for compose_file in docker_compose_files:
            logging.info(f"Running docker-compose down -v in {compose_file}...")
            os.chdir(os.path.dirname(compose_file))
            result = subprocess.run([self.docker_compose_path, "down", "-v"])
            if result.returncode != 0:
                logging.error(f"Error executing docker-compose down -v in {compose_file}.")
            else:
                logging.info(f"Successfully ran docker-compose down -v in {compose_file}")
...
if __name__ == "__main__":
    ...
    docker_compose_files = stopper.run_docker_compose_down_with_volumes()
